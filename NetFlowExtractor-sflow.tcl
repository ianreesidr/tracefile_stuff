...
proc indexCommonFields {AllFields} {
	set CommonFields {frame.number ip.src cflow.sequence cflow.source_id}
...
proc readPcap {cFlows} {
	...
	global sFlow
	if {$sFlow == 1} {
		# note that we changed the interface fields from sflow.flow_sample.input/output_interface to sflow.flow_sample.input/output_interface_value
	        set FlowFields {
        	        sflow_245.numsamples sflow.flow_sample.sequence_number sflow.flow_sample.input_interface_value sflow.flow_sample.output_interface_value eth.dst eth.src  eth.type ip.dsfield ip.flags ip.proto ip.src ip.dst udp.srcport udp.dstport tcp.srcport tcp.dstport sflow_245.header.frame_length frame.number
  		}
		set FlowType "sflow"
	} else {
        	set FlowFields {
        cflow.count cflow.octets cflow.packets  cflow.inputint  cflow.outputint  cflow.srcaddr  cflow.dstaddr  cflow.protocol  cflow.tos  cflow.timedelta  cflow.srcport cflow.dstport  cflow.timestart  cflow.timeend  cflow.sampler_id  cflow.flow_class  cflow.nexthop  cflow.dstmask  cflow.srcmask  cflow.tcpflags  cflow.direction  cflow.sequence cflow.source_id  ip.src frame.number
        	}
		set FlowType "cflow"
	}
